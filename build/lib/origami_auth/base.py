...
``AbstractOrigamiUser`` carries every field and role a Tarmar app's user needs,
...
that does *not* list ``origami_auth`` in ``INSTALLED_APPS`` — e.g. a deployed app
...
Greenfield apps instead use the concrete :class:`origami_auth.models.User`.
"""
...
class AbstractOrigamiUser(AbstractUser):
    ...
    Concrete subclasses (this library's :class:`~origami_auth.models.User`, or a
